        ...
        <nav className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-1 overflow-y-auto max-h-[45vh] pr-2 custom-scrollbar">
            {filteredCinemas.map((c, i) => (
              <div key={c.place_id} className="group/item relative">
                <button
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden pr-6">
                    ...
                    <span className="text-xs font-bold truncate lg:hidden">{(c.place_name || '').replace(/LOTTE Cinema\s*/gi, '').trim() || c.name || 'Unknown'}</span>
                    <span className="text-xs font-bold truncate hidden lg:block">{(c.place_name).replace(/LOTTE Cinema\s*/gi, '').trim() || 'Unknown'}</span>
                  </div>
