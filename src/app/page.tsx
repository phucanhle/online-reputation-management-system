...
export default async function Dashboard() {
  ...
  try {
    ...
    cinemas = await Promise.all(formattedCinemasList.map(async (c: any) => {
        // Find reviews by place_id since that's what the raw mongo documents have
        const branchReviews = await reviewsColl.find({ place_id: c.place_id } as any).sort({ review_date: -1, isoDate: -1 } as any).limit(50).toArray();
